    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
      ...
      <section className="py-20 px-6">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              { title: 'Hair Stylists', icon: '💇' },
              { title: 'Nail Artists', icon: '💅' },
              { title: 'Lash Technicians', icon: '👁️' },
              { title: 'Tattoo Artists', icon: '🎨' },
              { title: 'Estheticians', icon: '✨' },
              { title: 'Salon Owners', icon: '🏢' },
            ].map((prof, idx) => (
              <Card key={idx} className="p-6 text-center hover:shadow-xl transition cursor-pointer">
                <div className="text-5xl mb-4">{prof.icon}</div>
                <h3 className="text-lg font-semibold">{prof.title}</h3>
              </Card>
            ))}
          </div>
        </div>
